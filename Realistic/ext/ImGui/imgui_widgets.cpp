...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    float anim = FastFloatLerp(std::string(label).append("button.anim"), hovered, 0.0f, 1.f, 0.009f); 

    window->DrawList->AddRectFilled(bb.Min, bb.Max, GetColorU32(ImGuiCol_ChildBg), style.FrameRounding);
    window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(ImGuiCol_SliderGrab, anim), style.FrameRounding);
    ImU32 color = (hovered == true ? ImColor(255, 255, 255) : ImColor(GetColorU32(ImGuiCol_TextDisabled)));
...
bool ImGui::Tab(const char* Icon, const char* label, bool tab)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = CalcItemSize({ 140, 30 }, label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);
    ...
    float anim = FastFloatLerp(std::string(label).append("tab.anim"), tab, 0.0f, 1.f, 0.009f);
    RenderNavHighlight(bb, id);

    ImGui::GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, GetColorU32(ImGuiCol_Tab, anim), style.FrameRounding);

    ImU32 color = (tab == true ? ImColor(255, 255, 255) : ImColor(GetColorU32(ImGuiCol_TextDisabled)));
    ImGui::PushStyleColor(ImGuiCol_Text, color);
    RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
    ImGui::PopStyleColor();
    ImGui::PushStyleColor(ImGuiCol_Text, color);
     RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, Icon, NULL, &icon_size, ImVec2(0.1f, 0.68f), &bb);
     ImGui::PopStyleColor();
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags);
    return pressed;
}

bool ImGui::SubTab(const char* label, bool tab)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = CalcItemSize({ 85, 30 }, label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);
    ...
    float anim = FastFloatLerp(std::string(label).append("subtab.anim"), tab, 0.0f, 1.f, 0.009f);
    RenderNavHighlight(bb, id);
...
#include "../../src/Fonts.h"
bool ImGui::Hotkey(const char* label, int* key)
{
    ...
    const ImGuiID id = window->GetID(label);
    const float size = 210;


    const ImRect rect(window->DC.CursorPos, window->DC.CursorPos + ImVec2(size, 25));
    const ImRect clickable(window->DC.CursorPos + ImVec2(0, 0), window->DC.CursorPos + ImVec2(size, 25));
    ...
    bool hovered_mode = ItemHoverable(clickable, id);
    ...
    bool hovered_key = ItemHoverable(clickable, id);
    ...
    float anim = FastFloatLerp(std::string(label).append("hotkey.anim"), hovered_key, 0.0f, 1.f, 0.009f);

    window->DrawList->AddRectFilled(clickable.Min, clickable.Max, GetColorU32(ImGuiCol_WindowBg), style.FrameRounding);
    window->DrawList->AddRect(clickable.Min, clickable.Max, GetColorU32(ImGuiCol_SliderGrab, anim), style.FrameRounding); 
   // window->DrawList->AddText(rect.Min, GetColorU32(ImGuiCol_Text), label);
    ImU32 color = (hovered_key == true ? ImColor(255, 255, 255) : ImColor(GetColorU32(ImGuiCol_TextDisabled)));
    ImGui::PushStyleColor(ImGuiCol_Text, color);
    window->DrawList->AddText(ImVec2(clickable.Max.x - 30, clickable.Min.y + 7), ImColor(255, 255, 255), "\xef\x84\x9c");//ICON_FA_KEYBOARD
    RenderTextClipped(clickable.Min + ImVec2(8, 5), clickable.Max - ImVec2(24, -45), buf_display, NULL, NULL, ImVec2(0.0f, 0.0f));
    ImGui::PopStyleColor(); 
    return value_changed;
}

bool ImGui::Loading_Bar(const char* label, float value, const ImVec2& size_arg, ImColor bg_col, ImColor fg_col) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    window->DrawList->AddRectFilled(bb.Min, ImVec2(bb.Max.x, bb.Max.y), bg_col);
    window->DrawList->AddRectFilled(bb.Min, ImVec2(bb.Min.x * value, bb.Max.y), fg_col);
...
bool ImGui::Loading_Spinner(const char* label, float radius, int thickness, ImColor color) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
void ImGui::Checkbox_Item(const char* Label, bool* v)
{
	ImGui::BeginChild(std::string(Label).append("begin.child.item").c_str(), ImVec2((650 - 190) / 2, 32));
	{
...
void ImGui::Combo_Item(const char* Label, int* items, const char* preview[], int item)
{
    ImGui::BeginChild(std::string(Label).append("begin.child.item").c_str(), ImVec2((650 - 190) / 2, 32));
    {
        ImGui::SetCursorPos({ 10, 5 });
        ImU32 color = (IsItemHovered() == true ? ImColor(255, 255, 255) : ImColor(GetColorU32(ImGuiCol_TextDisabled)));
        ImGui::PushStyleColor(ImGuiCol_Text, color);
        ImGui::Text(Label); 
        ImGui::PopStyleColor(); 
        ImGui::SameLine(175); 
        ImGui::Combo(std::string(Label).append("combo").c_str(), items, preview, item); 
       
    }
    ImGui::EndChild();
}

void ImGui::InputText_Item(const char* Label, char* buf,size_t size, ImGuiInputTextFlags flags)
{
    ImGui::BeginChild(std::string(Label).append("begin.child.item").c_str(), ImVec2((650 - 190) / 2, 32));
    {
        ImGui::SetCursorPos({ 10, 5 });
        ImGui::InputText(Label, buf, size, flags);
    }
    ImGui::EndChild();
}


void ImGui::Hotkey_Item(const char* Label, int* v)
{
    ImGui::BeginChild(std::string(Label).append("hot.key.item").c_str(), ImVec2((650 - 190) / 2, 35));
    {
        ImGui::SetCursorPos({ 10, 5 });
        ImGui::Hotkey(Label, v);

    }
    ImGui::EndChild();
}

void ImGui::Checkbox_Item_Color(const char* Label, bool* v, float color[4])
{
    ImGui::BeginChild(std::string(Label).append("begin.color.item").c_str(), ImVec2((650 - 190) / 2, 32));
    {
        ImGui::SetCursorPos({ 10, 5 });
        ImGui::Checkbox(Label, v);
        ImGui::SameLine(185);
        ImGui::ColorEdit4(std::string(Label).append("color.Picker").c_str(), color, ImGuiColorEditFlags_NoSidePreview | ImGuiColorEditFlags_AlphaBar | ImGuiColorEditFlags_NoInputs);
    }
    ImGui::EndChild();
}

void ImGui::Slider_Item(const char* Label, int* slider, int min, int max)
{
    ImGui::BeginChild(std::string(Label).append("begin.child.item").c_str(), ImVec2((650 - 190) / 2, 32));
    {
        ImGui::SetCursorPos({ 10, 5 });
        ImGui::SliderInt(Label, slider, min, max); 
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    float anim = FastFloatLerp(std::string(label).append("checkbox.anim"), *v, 0.f, 1.f, 0.01f);
    float animhov = FastFloatLerp(std::string(label).append("checkbox.animhov"), *v, 0.f, 1.f, 0.01f);

    const ImRect check_bb(pos, pos + ImVec2(square_sz, square_sz));
    RenderNavHighlight(total_bb, id);

    window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, GetColorU32(ImGuiCol_WindowBg), style.FrameRounding);
   // window->DrawList->AddRect(check_bb.Min, check_bb.Max, GetColorU32(ImGuiCol_SliderGrab, animhov), style.FrameRounding);
    window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, GetColorU32(ImGuiCol_SliderGrab, anim), style.FrameRounding);
    const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));
    RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad, pad), GetColorU32(ImGuiCol_Text, anim), square_sz - pad * 2.0f);

    ImVec2 label_pos = ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + style.FramePadding.y);

    ImU32 color = (*v == true ? ImColor(255, 255, 255) : ImColor(GetColorU32(ImGuiCol_TextDisabled)));
    ImGui::PushStyleColor(ImGuiCol_Text, color);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    float popup_open_anim = FastFloatLerp(std::string(label).append("begincombo.popup_open_anim"), popup_open, 0.f, 1.f, 0.009f);
    float comboanimhov = FastFloatLerp(std::string(label).append("begincombo.hovanim"), hovered, 0.f, 1.f, 0.009f); ;

 
     ImU32 color = (popup_open == true ? ImColor(255, 255, 255) : ImColor(GetColorU32(ImGuiCol_TextDisabled)));
    RenderArrow(window->DrawList, { clickable.Max.x - 27 - style.FramePadding.x, clickable.GetCenter().y - 13 }, color, ImGuiDir_Down, 20.f);
    ...
    else
    {
        ...
        SetNextWindowSizeConstraints(ImVec2(w, 0.0f), ImVec2(FLT_MAX, CalcMaxPopupHeightFromItemCount(popup_max_height_in_items) * popup_open_anim));
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(frame_bb.Max.x, frame_bb.Max.y), ImColor(33, 33, 33), style.FrameRounding);
    window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(frame_bb.Min.x + it_filing->second.value + 14, frame_bb.Max.y), GetColorU32(ImGuiCol_SliderGrab), style.FrameRounding);


    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
    ImU32 color = (IsItemActive() == true ? ImColor(255, 255, 255) : ImColor(GetColorU32(ImGuiCol_TextDisabled)));
    ImGui::PushStyleColor(ImGuiCol_Text, color);
...
#include <windows.h>
inline ImVec2 M_Size = ImVec2(GetSystemMetrics(SM_CXSCREEN), GetSystemMetrics(SM_CYSCREEN));
...
bool ImGui::SliderScalar_2(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, 25));
    const ImRect frame_bb(total_bb.Min + ImVec2(0, label_size.y), total_bb.Max);
...
bool ImGui::Loading_Slider(const char* label, int* v, int v_min, int v_max, const char* format, ImGuiSliderFlags flags)
{
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize({ CalcItemWidth(), 32 - 10 }, CalcItemWidth(), (is_multiline ? g.FontSize * 8.0f : label_size.y) + style.FramePadding.y * 2.0f); // Arbitrary default of 8 lines high for multi-line
    ...
    {
        RenderNavHighlight(frame_bb, id);
        RenderFrame(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_WindowBg), true, style.FrameRounding);
        //	window->DrawList->AddLine( { frame_bb.Min.x, frame_bb.Min.y } , { frame_bb.Min.x, frame_bb.Max.y } , ImColor( 210 , 210 , 210 ) , 2 ); 
    }
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(it_filing->second.value + 0, 6), frame_bb.Min + ImVec2(it_filing->second.value + 1, 18), GetColorU32(ImGuiCol_Text));
    ...
    if (!render_cursor)
        RenderText(ImVec2(frame_bb.Min.x + 4, frame_bb.Min.y + style.FramePadding.y), label);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    it_nameanimX->second = ImLerp(it_nameanimX->second, it_nameanimX->second ? square_sz * 11 - 10 : 0.f, g.IO.DeltaTime * 6.f);
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        SetNextWindowSize(ImVec2(it_nameanimX->second + 15, 270));
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        draw_list->AddCircleFilled(hue_cursor_pos, 3, hue_color32, hue_cursor_segments);
        draw_list->AddCircle(hue_cursor_pos, 3 + 1, col_midgrey, hue_cursor_segments);
        draw_list->AddCircle(hue_cursor_pos, 3, col_white, hue_cursor_segments);
    ...
    draw_list->AddCircleFilled(sv_cursor_pos, 5, user_col32_striped_of_alpha, 12);
    draw_list->AddCircleFilled(sv_cursor_pos, 5 + 1, col_midgrey, 12);
    draw_list->AddCircleFilled(sv_cursor_pos, 5, col_white, 12);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    const float default_size = GetFrameHeight();
    const ImVec2 size(35, GetFrameHeight());
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + size);
    ...
    if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.w < 1.0f)
    {
        ...
        RenderColorRectWithAlphaCheckerboard(window->DrawList, ImVec2(bb_inner.Min.x + grid_step, bb_inner.Min.y), bb_inner.Max, GetColorU32(col_rgb), grid_step, ImVec2(-grid_step + off, off), 0, ImDrawFlags_RoundCornersRight);
        window->DrawList->AddRectFilled(bb_inner.Min, ImVec2(mid_x, bb_inner.Max.y), GetColorU32(col_rgb_without_alpha), 0, ImDrawFlags_RoundCornersLeft);
    }
    else
    {
        ...
        if (col_source.w < 1.0f)
            RenderColorRectWithAlphaCheckerboard(window->DrawList, bb_inner.Min, bb_inner.Max, GetColorU32(col_source), grid_step, ImVec2(off, off), 0);
        else
            window->DrawList->AddRectFilled(bb_inner.Min, bb_inner.Max, GetColorU32(col_source), 0);
    }
    RenderNavHighlight(bb, id);
    if ((flags & ImGuiColorEditFlags_NoBorder) == 0)
    {
        if (g.Style.FrameBorderSize > 0.0f)
            ...
            window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(ImGuiCol_FrameBg), 0); // Color button are often in need of some sort of border
    }
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    float anim = FastFloatLerp(std::string(label).append("selectable"), selected, 0.0f, 1.0f, 0.009f); 
    window->DrawList->AddRectFilled(bb.Min, bb.Max, GetColorU32(ImGuiCol_SliderGrab, anim), 0); 
