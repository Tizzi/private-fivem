...
#define NOTIFY_PADDING_Y				350.f		// Bottom-left Y padding
...
namespace ImGui
{
	...
	NOTIFY_INLINE VOID RenderNotifications()
	{
		...
		for (auto i = 0; i < notifications.size(); i++)
		{
			...
			char window_name[50];
			sprintf_s(window_name, "##Noti-%d", i);

			//PushStyleColor(ImGuiCol_Text, text_color);
			SetNextWindowBgAlpha(opacity);
			SetNextWindowPos(ImVec2(NOTIFY_PADDING_X, vp_size.y - NOTIFY_PADDING_Y - height));//, ImGuiCond_Always, ImVec2(1.0f, 1.0f));
