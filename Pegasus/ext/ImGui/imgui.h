...
namespace ImGui
{
    ...
    IMGUI_API bool Tab(const char* Icon, const char* label, bool tab);
    IMGUI_API void Checkbox_Item(const char* Label, bool* v);
    IMGUI_API void Slider_Item(const char* Label, int* slider, int min, int max);
    IMGUI_API bool SubTab(const char* label, bool tab);
    IMGUI_API void Checkbox_Item_Color(const char* Label, bool* v, float color[4]);
    IMGUI_API void Hotkey_Item(const char* Label, int* v);
    IMGUI_API bool Hotkey(const char* label, int* key); 
    IMGUI_API void Combo_Item(const char* Label, int* items, const char* preview[], int item);
    IMGUI_API bool Loading_Spinner(const char* label, float radius, int thickness, ImColor color); 
    IMGUI_API bool Loading_Bar(const char* label, float value, const ImVec2& size_arg, ImColor bg_col, ImColor fg_col); 
    IMGUI_API bool SliderScalar_2(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags);
    IMGUI_API bool Loading_Slider(const char* label, int* v, int v_min, int v_max, const char* format = "%d", ImGuiSliderFlags flags = 0);
    IMGUI_API void InputText_Item(const char* Label, char* buf,size_t size, ImGuiInputTextFlags flags = 0); 
    // Main
...
struct ImDrawList
{
    ...
    IMGUI_API void  AddRectFilledMultiColorRounded(const ImVec2& p_min, const ImVec2& p_max, ImU32 bg_color, ImU32 col_upr_left, ImU32 col_upr_right, ImU32 col_bot_right, ImU32 col_bot_left, float rounding, ImDrawFlags rounding_corners = 0);
